<template>
	<view>
		<view class="echarts" id="fgf" :prop="option" :change:prop="$echarts.update" @click="$echarts.onClick"></view>
	</view>
</template>
...
<script module="echarts" lang="renderjs">
	export default {
		...
		methods: {
			...
			init() {
				// 根据id初始化图表
				this.chart = this.$echarts.init(document.getElementById(this.option.id))
					...
					if (option) {
						...
						if (option.series) {
							for (let i in option.series) {
								...
								if (linearGradient) {
									option.series[i].color = new this.$echarts.graphic.LinearGradient(linearGradient[0],linearGradient[1],linearGradient[2],linearGradient[3],linearGradient[4])
								}
							}
						}
					}
