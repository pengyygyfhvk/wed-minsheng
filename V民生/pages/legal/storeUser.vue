				...
				<view class="gray75 flex column w100 plr15 ptb15 bdb27" v-for="(item,index) in orderlist" :key="index" @click="goDetail(item.id,item.is_sure)">
					<view class="flex between">
						<view class=" flex alcenter ft13 bold">
							<text :class="['blue21 mr5 ft14',{'redColor':item.type=='sell'}]">{{item.type=='sell' ? $t('legal.sell'):$t('legal.buy')}}</text>
			...
			<view class="bgPart" @click.stop>
				<view class="pt10 plr10 ft10 ">
					...
					<view class="select deal_type mt10  mb10 flex">
						<view :class="type=='sell'?'active':'baseBg'" @click="changetype('sell')">{{$t('legal.buy')}}</view>
						<view :class="type=='buy'?'active':'baseBg'" @click="changetype('buy')">{{$t('legal.sell')}}</view>
					</view>
	...
	export default {
		...
		methods: {
			getData() {
				...
				that.$utils.initDataToken({url: 'seller_legal_user_deal',
					data: {
						page:that.page,
						currency_id: that.id,
						is_sure:is_sure
